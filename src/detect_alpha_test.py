...
class DetectBeta():
	def __init__(self):
		...
		# Initialize node
		rospy.init_node('detect_beta', anonymous=True)
	...
	def update_Chan5(self, line2):
		
		line5.set_ydata(np.array(self.list5[-202:-1]))
		ax[0].relim()
		ax[0].autoscale_view(False,True,True)
		line5.figure.canvas.draw()


	def update_Chan6(self, line2):
		
		line6.set_ydata(np.array(self.list6[-202:-1]))
		ax[1].relim()
		ax[1].autoscale_view(False,True,True)
		line6.figure.canvas.draw()


	def update_Chan7(self, line2):
		
		line7.set_ydata(np.array(self.list7[-202:-1]))
		ax[2].relim()
		ax[2].autoscale_view(False,True,True)
		line7.figure.canvas.draw()


	def update_Chan8(self, line):

		line8.set_ydata(np.array(self.list8[-202:-1]))
		ax[3].relim()
...
if __name__ == '__main__':
    try:
		...
		li5, = ax[0].plot(np.linspace(0, 201, 201), np.linspace(0, 201, 201))
		ax[0].set_title("CHANNEL 5")
		
		li6, = ax[1].plot(np.linspace(0, 201, 201), np.linspace(0, 201, 201))
		ax[1].set_title("CHANNEL 6")
		
		li7, = ax[2].plot(np.linspace(0, 201, 201), np.linspace(0, 201, 201))
		ax[2].set_title("CHANNEL 7")
		
		li8, = ax[3].plot(np.linspace(0, 201, 201), np.linspace(0, 201, 201))
		ax[3].set_title("CHANNEL 8")
